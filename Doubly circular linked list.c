...
int insertNode(listNode* h, int key) { // 주석에 ?? -> ?? 값 이렇게 되어있으면 ??가 ??를 가리키는 값이라고 생각하면 됩니다.
	...
	listNode *node = (listNode*)malloc(sizeof(listNode)); // listNode 포인터 변수 node 메모리 할당
	listNode* p; // listNode 포인터 변수 p 생성
	p = h; // p에 헤더노드 대입
	node -> key = key; // node -> key 값에 key 대입
	node -> rlink = NULL; // node -> rlink 값에 NULL 대입
	node -> llink = NULL; // node -> llink 값에 NULL 대입
	...
	while(1)
	{
		...
		if (p == h) // p가 헤더노드일 경우
		{
			node -> rlink = h; // node -> rlink 값에 헤더노드 대입
			node -> llink = h; // node -> llink 값에 헤더노드 대입
			h -> rlink = node; // node -> rlink 값에 node 대입
			h -> llink = node; // 헤더노드 -> llink 값에 node 대입
			break;
		}
		else if (p -> key >= node -> key) // p -> key 값이 node -> key 값보다 크거나 같을 경우
		{
			node -> rlink = p; // node -> rlink 값에 p 대입
			node -> llink = p -> llink; // node -> llink 값에 p -> llink 값 대입
			p -> llink -> rlink = node; // p가 가리키는 llink 값의 가리키는 rlink 값에 node 대입
			p -> llink = node; // p -> llink 값에 node 대입
			break;
		}
		else if (p -> rlink == h) // p -> rlink 값이 헤더노드이면
		{
			node -> rlink = p -> rlink; // node -> rlink 값에 p -> rlink 대입
			node -> llink = p; // node -> llink 값에 p 대입
			p -> rlink -> llink = node; // p가 가리키는 rlink 값에 가리키는 llink 값에 node 대입
